...
my $size_used_high = '1.3.6.1.4.1.2021.9.1.16';

# Globals

my $Name='check_snmp_storage_ucd';
my $Version='1.4.0';
...
sub print_usage {
    print "Usage: $Name [-v] -H <host> -C <snmp_community> [-2] | (-l login -x passwd [-X pass -L <authp>,<privp>]) [-p <port>] -m <name in desc_oid> [-q storagetype] -w <warn_level> -c <crit_level> [-t <timeout>] [-T pl|pu|bl|bu ] [-r -i -G] [-e] [-S 0|1[,1,<car>]] [-o <octet_length>] [-R <% reserved>]\n";
}
...
sub help {
   ...
   print "(c)2004-2016 Patrick Proy\n\n";
   print_usage();
   print <<EOT;
...
foreach my $key ( keys %$resultat) {
  ...
  if ($test) {
	...
	if ($test) {
	   ...
	   $oids[$count_oid++]=$size_total_low . '.' . $tindex[$num_int];
	   $oids[$count_oid++]=$size_total_high . '.' . $tindex[$num_int];
	   $oids[$count_oid++]=$size_used_low . '.' . $tindex[$num_int];
	   $oids[$count_oid++]=$size_used_high . '.' . $tindex[$num_int];
...
my $used=0;
my $output=undef;
my $output_metric_val = 1000;
my $output_metric = "M";
# Set the metric 
if (defined($o_giga)) {
	$output_metric_val = 1000000;
	$output_metric='G';
}

for ($i=0;$i<$num_int;$i++) {
  verb("Descr : $descr[$i]");
  verb("Total Low :  $$result{$size_total_low . '.' . $tindex[$i]}");
  verb("Total High :  $$result{$size_total_high . '.' . $tindex[$i]}");
  verb("Used Low :  $$result{$size_used_low . '.' . $tindex[$i]}");
  verb("Used High :  $$result{$size_used_high . '.' . $tindex[$i]}");

  if (!defined($$result{$size_total_low . '.' . $tindex[$i]}) || 
	!defined($$result{$size_total_high . '.' . $tindex[$i]}) || 
	!defined($$result{$size_used_low . '.' . $tindex[$i]}) || 
	!defined ($$result{$size_used_high . '.' . $tindex[$i]})) {
     print "Data not fully defined for storage ",$descr[$i]," : UNKNOWN\n";
     exit $ERRORS{"UNKNOWN"};
  }
  
  # total size in KB
  my $total_size = (($$result{$size_total_high . '.' . $tindex[$i]} * 4294967296) + $$result{$size_total_low . '.' . $tindex[$i]}) / $output_metric_val;
  # used size in KB
 ...
 if ($o_type eq "pu") { # Checks % used
  ...
  if ($o_type eq 'bl') { # Checks MBytes left
    my $locstate=0;
